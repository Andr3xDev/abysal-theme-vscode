import { AbysalThemes, colors } from './colors'
...
export function createThemeHelpers({ color, soft = false, black = false }: GetThemeOptions) {
  ...
  const v = (key: keyof typeof AbysalThemes, op = '') => {
    let obj = black
      ? AbysalThemes[`black${capitalize(key)}` as keyof typeof AbysalThemes] || AbysalThemes[key]
      : soft
        ? AbysalThemes[`soft${capitalize(key)}` as keyof typeof AbysalThemes] || AbysalThemes[key]
        : AbysalThemes[key]
